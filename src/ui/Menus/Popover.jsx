...
export function Popover({trigger, side, children, onOpenChanged}) {
    ...
    return (
        <ControlledPopover trigger={trigger} side={side} open={open} setOpen={setOpen}>
                           {children}
        </ControlledPopover>
    );
}

export function ControlledPopover({trigger, side = 'bottom', open, setOpen, children}) {
                ...
                {open ? (
                    <BasePopover.Portal forceMount>
                        <BasePopover.Content asChild side={side} sideOffset={5}>
                            ...
                            >
                                {clonedChildren.map((clonedChild) => (
                                    clonedChild
                                ))}
                            </Content>
                        </BasePopover.Content>
